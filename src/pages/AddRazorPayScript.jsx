...
async function AddRazorPayScript()  {
    ...
    var options = {
        ...
        "handler": async function (response) {
            ...
            const data_pdf = await fetch('https://payment-ultron-official.netlify.app/.netlify/functions/api/create/pass', {
                method: 'POST',
                // mode: 'no-cors',
                body: JSON.stringify({
                    OrderID:response.razorpay_order_id,
                ...
                const fetch_pdf = await fetch('https://payment-ultron-official.netlify.app/.netlify/functions/api/fetch-pdf', {
                    method: 'GET',
                    // mode: 'no-cors',
                    body: JSON.stringify({
                        bgUrl: bgUrl,
                ...
                }).then((t) =>t.blob()).then((myBlob) => {
                    saveAs(myBlob, 'newPdf.pdf');
                  });
            }
            );
            console.log(data);
